...
```{r yrep,fig.cap="Comparing the posterior predicted distribution of observed viral loads with what is seen in the data",cache=TRUE}
...
readr::read_csv("../../incidence-from-prevalence/ataccc_sims/2022-02_informative-prior/integration_samples_monte_carlo.csv",
  ...
  bind_rows(tbl_duration_unvacc %>% filter(model == "simplified")) %>% 
  ggplot(aes(t, F)) +
  stat_lineribbon(aes(fill = model), .width = 0.95, alpha = 0.25, size = 0.7)
```
<!--
